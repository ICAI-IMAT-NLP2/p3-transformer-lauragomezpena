...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        hidden_state =  self.attention(norm_x, norm_x, norm_x, mask)
        x = x + hidden_state
        
        # Apply layer normalization and then apply feed-forward network
        ff_output = self.feed_forward(self.layer_norm_2(x)) 
        x = x + ff_output
        return x
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, vocab_size: int, max_position_embeddings: int, d_model: int,
        ...
        self.layers =  nn.ModuleList([TransformerEncoderLayer(d_model=d_model, num_attention_heads=num_attention_heads, intermediate_size=intermediate_size) for i in range(num_hidden_layers)])
